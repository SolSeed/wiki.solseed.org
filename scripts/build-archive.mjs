...
const css = `:root{font-family:system-ui,sans-serif;line-height:1.5}body{max-width:78rem;margin:auto;padding:1rem}a{color:#05c}nav ul{display:flex;flex-wrap:wrap;gap:.75rem 1.5rem;padding:0;list-style:none}.metadata{display:grid;grid-template-columns:max-content minmax(0,1fr);gap:.25rem 1rem}.metadata dt{font-weight:bold}.metadata dd{margin:0}.wiki,.source{white-space:pre-wrap;overflow-wrap:anywhere;overflow:auto;border:1px solid #888;padding:1rem}.table-wrap{overflow-x:auto}table{border-collapse:collapse;width:100%}th,td{border:1px solid #888;padding:.4rem;text-align:start;vertical-align:top}fieldset{display:flex;flex-wrap:wrap;gap:.5rem;align-items:center}.help{flex-basis:100%}.diff-output{min-height:8rem;border:1px dashed #888;padding:1rem}`;
const controls = `const shell=document.querySelector('[data-diff-shell]');const form=document.querySelector('[data-diff-controls]');const status=document.querySelector('[data-diff-status]');const params=new URLSearchParams(location.search);const from=params.get('from'),to=params.get('to');if(form&&from)form.elements.from.value=from;if(form&&to)form.elements.to.value=to;if(shell){fetch('/archive-data/pages/'+encodeURIComponent(shell.dataset.pageId)+'/page.json').then(r=>{if(!r.ok)throw Error();return r.json()}).then(page=>{const ids=new Set((page.revisions||[]).map(r=>String(r.revision_id)));if((from&&!ids.has(from))||(to&&!ids.has(to))){if(status)status.textContent='That revision pair is not public for this page.'}else if(from&&to&&status)status.textContent='Revision pair validated. A diff renderer will be integrated here.'}).catch(()=>{if(status)status.textContent='Unable to validate public revision metadata.'})}`;
...
async function loadPage(source, pageId) {
  ...
  for (const compact of page.revisions) {
    ...
    const merged = { ...compact, ...metadata, revision_id: revisionId, wikitext_bytes: bytes, wikitext_sha256: digest };
...
export async function buildArchive({ sourceDir = "_source", outputDir = "site" } = {}) {
  ...
  for (const generated of ["index.html", "history", "revision", "diff", "source", "archive-data", "assets/history.css", "assets/history-controls.js"]) await rm(join(output, generated), { recursive: true, force: true });
